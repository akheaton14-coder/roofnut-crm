...
export default function PipelinePage(){
  ...
  async function drop(event:DragEvent,stageId:string,openPhase?:string){event.preventDefault();const jobId=event.dataTransfer.getData("jobId");if(!jobId)return;setJobs(current=>current.map(job=>job.id===jobId?{...job,workflow_stage_id:stageId}:job));await supabase.from("jobs").update({workflow_stage_id:stageId,updated_at:new Date().toISOString()}).eq("id",jobId);if(openPhase)setActivePhase(openPhase)}
  ...
  const activePhaseIndex=phases.findIndex(phase=>phase.id===activePhase),previousPhase=activePhaseIndex>0?phases[activePhaseIndex-1]:null,nextPhase=activePhaseIndex>=0&&activePhaseIndex<phases.length-1?phases[activePhaseIndex+1]:null;
  const transferPanel=(phase:Phase,direction:"previous"|"next")=><aside className={`phase-transfer ${direction}`}><button className="transfer-head" onClick={()=>setActivePhase(phase.id)}><small>{direction==="next"?"NEXT STAGE":"PREVIOUS STAGE"}</small><b>{direction==="next"?`${phase.label} →`:`← ${phase.label}`}</b><span>Drag onto the exact status</span></button><div>{stages.filter(phase.matches).map(stage=><section key={stage.id} onDragOver={e=>e.preventDefault()} onDrop={e=>drop(e,stage.id,phase.id)} style={{borderColor:stage.color}}><i style={{background:stage.color}}/><b>{stage.name}</b><span>{jobs.filter(job=>job.workflow_stage_id===stage.id).length}</span><small>Drop here</small></section>)}</div></aside>;
  ...
  return <CrmShell userName={userName}><div className="board-page">
    ...
    <div className="kanban focused">{previousPhase&&transferPanel(previousPhase,"previous")}{boardStages.map(stage=>{const cards=jobs.filter(job=>job.workflow_stage_id===stage.id);return <section className="kanban-column" key={stage.id} onDragOver={e=>e.preventDefault()} onDrop={e=>drop(e,stage.id)}><header style={{borderTopColor:stage.color}}><div><b>{stage.name}</b><span>{cards.length}</span></div><p>${cards.reduce((sum,j)=>sum+Number(j.contract_value),0).toLocaleString()}</p></header><div className="kanban-cards">{cards.map(job=>{const days=Math.floor((Date.now()-new Date(job.updated_at).getTime())/86400000);return <Link href={`/jobs/${job.id}`} draggable onDragStart={e=>e.dataTransfer.setData("jobId",job.id)} className="kanban-card" key={job.id}><div className="card-top"><b>{job.title}</b>{stage.time_limit_days&&days>stage.time_limit_days&&<span>Stuck {days}d</span>}</div><p>{job.clients?.first_name} {job.clients?.last_name} · {job.properties?.city}</p><footer><b>${Number(job.contract_value).toLocaleString()}</b><span>{job.next_action||"Add next action"}</span></footer></Link>})}<div className="drop-hint">Drop job here</div></div></section>})}{nextPhase&&transferPanel(nextPhase,"next")}</div>
  </div></CrmShell>
}
